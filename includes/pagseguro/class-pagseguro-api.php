 ...
 * API.
 */
class WC_AntFraude_PagSeguro_API {
	 ...
	 * @return array
	 */
	public function do_payment_request( $order, $posted ) { // TODO: Verificar
		$payment_method = isset( $posted['pagseguro_payment_method'] ) ? $posted['pagseguro_payment_method'] : '';
		//error_log(print_r($this->get_available_payment_methods(), true));die;
		/**
		...
		//error_log(print_r($order, true)); //  TODO: Area de teste
		//error_log(print_r($posted, true));die; // TODO: Area de teste
		// Sets the xml.
		...
		if ( is_wp_error( $response ) ) {
			...
			try {
			 ...
			 //error_log(print_r($data->code[0][0], true));die; // TODO: Area de teste
