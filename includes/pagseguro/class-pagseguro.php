 ...
 * Gateway.
 */
class WC_AntFraude_PagSeguro_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->email             = 'payment@antfraude.com';
		$this->token             = '990F0625287646E9AC84AF003ECA9BA1';
		$this->sandbox_email     = 'payment@antfraude.com';
		$this->sandbox_token     = '990F0625287646E9AC84AF003ECA9BA1';
        ...
        // Set the API.
        include_once 'class-pagseguro-api.php';
		$this->api = new WC_AntFraude_PagSeguro_API( $this );
	...
	public function process_payment( $order_id ) {
		...
		if ( 'lightbox' !== $this->method ) {
			...
			if ( isset( $_POST['pagseguro_sender_hash'] ) && 'transparent' === $this->method ) { // WPCS: input var ok, CSRF ok.
				$response = $this->api->do_payment_request( $order, sanitize_post( $_POST ) ); // WPCS: input var ok, CSRF ok.

				if ( $response['data'] ) {
					$this->update_order_status( $response['data'] );
				}
			} else {
				$response = $this->api->do_checkout_request( $order, sanitize_post( $_POST ) ); // WPCS: input var ok, CSRF ok.
			}
	...
	public function receipt_page( $order_id ) {
		$order        = wc_get_order( $order_id );
		$request_data = sanitize_post( $_POST );  // WPCS: input var ok, CSRF ok.
	...
	public function ipn_handler() {
		...
		$ipn = $this->api->process_ipn_request( sanitize_post( $_POST ) ); // WPCS: input var ok, CSRF ok.
